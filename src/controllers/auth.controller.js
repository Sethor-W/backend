...
export class AuthController {
    ...
    static async register(req, res) {
        try {
            ...
            const newProfile = await Profile.create({name, codeUser: generateUniqueCode(), lastname, rut, phone, userId: newUser.id });

            // Generar token de autenticación
            const token = signJWT({ userId: newUser.id });

            return sendResponse(res, 201, false, 'Registrado', null, { token: token });
        } catch (error) {
    ...
    static async login(req, res) {
        try {
            ...
            return sendResponse(res, 201, false, 'Acceso exitoso', null, { token: token });
        } catch (error) {
